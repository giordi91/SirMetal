...
class Clock final {
 public:
  ...
  // it is thread safe
  static inline high_res_time_point now() {
...
template <typename T>
inline long long Clock<T>::getTicks() const {
  ...
  return m_running
             ? std::chrono::duration_cast<T>(now().time_since_epoch()).count()
             : std::chrono::duration_cast<T>(m_past.time_since_epoch()).count();
}

template <typename T>
long long Clock<T>::getDelta() {
  ...
  const long long duration =
      std::chrono::duration_cast<T>(curr - m_past).count();
  m_past = curr;
  return duration;
}
...
template <typename T>
long long Clock<T>::getDeltaFromOrigin() const {
  const high_res_time_point curr = m_running ? now() : m_past;
  const long long duration =
      std::chrono::duration_cast<T>(curr - m_start_time).count();
  return duration;
}

template <typename T>
void Clock<T>::sleep(const long long amount) const {
