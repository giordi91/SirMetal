...
const char *StringPool::loadFileFrame(const char *path,
                                      uint32_t &readFileSize) {
  FILE *fp = fopen(path, "rb");
  ...
  // allocating memory
  char *buffer =
      reinterpret_cast<char *>(m_stackAllocator.allocate(fileSize + 1));
...
const char *StringPool::concatenateFrame(const char *first, const char *second,
  ...
  // make the allocation
  char *newChar = reinterpret_cast<char *>(m_stackAllocator.allocate(totalLen));
  // do the memcpy
  memcpy(newChar, first, firstLen);
  if (joinerLen != 0) {
    memcpy(newChar + firstLen, joiner, joinerLen);
...
const char *StringPool::convert(const wchar_t *string, const uint8_t flags) {
  ...
  // make the allocation
  auto *newChar =
      reinterpret_cast<char *>(m_pool.allocate(len + 1, allocFlags));
  // do the conversion
...
const wchar_t *StringPool::convertWide(const char *string,
  ...
  // do the conversion
  mbstowcs(newChar, string, (len + 1) * sizeof(wchar_t));
