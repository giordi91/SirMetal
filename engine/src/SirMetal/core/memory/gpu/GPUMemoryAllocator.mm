...
namespace SirMetal {
    ...
    void GPUMemoryAllocator::update(BufferHandle handle, void *data, uint32_t offset, uint32_t size) const {
        ...
        {
            printf("[ERROR] Tried to update buffer %i, but buffer could not be found",index);
            return;
        }
    ...
    id GPUMemoryAllocator::getBuffer(BufferHandle handle) {
        ...
        if(found == m_bufferStorage.end())
        {
            printf("[ERROR] Tried to get metal buffer from handle %i, but buffer could not be found",index);
            return nil;
        }
