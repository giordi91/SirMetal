...
TEST_CASE("hashmap insert ", "[memory]") {
  SirMetal::HashMap<uint32_t, uint32_t, SirMetal::hashUint32> alloc(200);
...
TEST_CASE("hashmap psudo random insert 1000", "[memory]") {
  SirMetal::HashMap<uint32_t, uint32_t, SirMetal::hashUint32> alloc(2000);
...
TEST_CASE("hashmap empty 1000", "[memory]") {
  SirMetal::HashMap<uint64_t, uint32_t, SirMetal::hashUint64> alloc(200);
  const int count = 1000;
  uint32_t value;
  for (int i = 0; i < count; ++i) {
    uint32_t k = rand();
...
TEST_CASE("hashmap empty 2000", "[memory]") {
  SirMetal::HashMap<uint64_t, uint32_t, SirMetal::hashUint64> alloc(4600);
  const int count = 1000;
  uint32_t value;
  for (int i = 0; i < count; ++i) {
    uint32_t k = rand();
